 ...
 public class Absent_EmpController
   implements Initializable
 {
   ...
   @FXML
   private TableView<Staff> StudentsTable;
   @FXML
...
/*  75 */     this.StudentsTable.setItems(Management.getPersonsList());
/*  76 */     this.NameColumn.setCellValueFactory(cellData -> ((Staff)cellData.getValue()).getPId().NameProperty());
     
/*  78 */     this.MA = new Management();
   }
   
   @FXML
   public void Search() {
/*  83 */     if (!this.SearchQuery.getText().equals("")) {
/*  84 */       ObservableList<Staff> TempList = FXCollections.observableArrayList(this.MA.getActiveTeachers());
/*  85 */       this.StudentsTable.getItems().clear();
/*  86 */       for (int i = 0; i < TempList.size(); i++) {
/*  87 */         if (!((Staff)TempList.get(i)).getPId().getName().contains(this.SearchQuery.getText())) {
...
/* 104 */     this.StudentsTable.setItems(Management.getPersonsList());
...
/* 113 */     this.st = (Staff)this.StudentsTable.getSelectionModel().getSelectedItem();
...
/* 149 */       alert.showAndWait();
   ...
   @FXML
/* 157 */   public void handleClose() { Management.getDialogStage().close(); }
 }


/* Location:              C:\Users\Abdo\Documents\Amgad-H.jar!\Controller\Absent_EmpController.class
