...
public class EditTeacherController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        TeachersTable.setItems(TeachingStaff.getPersonsList());
    ...
    public void Search() {
        if (!SearchQuery.getText().equals("")) {
            ObservableList<Teacher> TempList = FXCollections.observableArrayList(TS.getTeachers());
            TeachersTable.getItems().clear();
    ...
    public void handleAbsentReport() {
        try {
            ...
            int selectedIndex = TeachersTable.getSelectionModel().getSelectedIndex();
            if (selectedIndex >= 0) {
                JasperReport jasperReport = JasperCompileManager.compileReport(StudReportsController.class.getClassLoader().getResourceAsStream("Reports/AbsentEmp.jrxml"));
                ...
                LOGGER.debug("inside printing Teacher Absent");
                ...
                pdfx.close();
                LOGGER.debug("done printing Teacher Absent");
            } else {
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setTitle("يوجد خطأ");
                alert.setHeaderText("لم يتم تحديد العنصر المراد تعديله");
    ...
    public void handleEvaluationReport() {
        try {
            ...
            int selectedIndex = TeachersTable.getSelectionModel().getSelectedIndex();
            if (selectedIndex >= 0) {
                JasperReport jasperReport = JasperCompileManager.compileReport(EditTeacherController.class.getClassLoader().getResourceAsStream("Reports/empEval.jrxml"));
                HashMap<String, Object> parameters = new HashMap<String, Object>();
                parameters.put("Eid", TeachersTable.getItems().get(selectedIndex).getPId().getPId().intValue());
                String dbUrl = prop.getProperty("dbUrl");
                String dbDriver = prop.getProperty("dbDriver");
                ...
                String dbPwd = prop.getProperty("dbPwd");
                Class.forName(dbDriver);
                LOGGER.debug("inside printing Teacher Evaluation");
                Connection conn = DriverManager
                        .getConnection(dbUrl, dbUname, dbPwd);
                JasperPrint print = JasperFillManager.fillReport(jasperReport, parameters, conn);
                ...
                TimeUnit.SECONDS.sleep(5);
                pdfx.close();
                LOGGER.debug("done printing Teacher Evaluation");
            } else {
                Alert alert = new Alert(Alert.AlertType.WARNING);
                alert.setTitle("يوجد خطأ");
                alert.setHeaderText("لم يتم تحديد العنصر المراد تعديله");
